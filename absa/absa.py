...
class ABSADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure length does not exceed BERT's maximum length
        max_len = 512
        if len(bert_tokens) > max_len - 2:
            bert_tokens = bert_tokens[:max_len - 2]
            bert_att = bert_att[:max_len - 2]
        
        segment_tensor = [0] + [0]*len(bert_tokens) + [0] + [1]*len(bert_att)
        bert_tokens = ['[cls]'] + bert_tokens + ['[sep]'] + bert_att

        if len(segment_tensor) > max_len:
            segment_tensor = segment_tensor[:max_len]
        if len(bert_tokens) > max_len:
            bert_tokens = bert_tokens[:max_len]

        bert_ids = self.tokenizer.convert_tokens_to_ids(bert_tokens)

        # Padding if necessary
        if len(bert_ids) < max_len:
            bert_ids += [0] * (max_len - len(bert_ids))
            segment_tensor += [0] * (max_len - len(segment_tensor))
...
class ABSAModel ():
    def __init__(self, tokenizer, adapter=True):
        self.model = ABSABert(r"C:\Users\mrtnr\Desktop\deneme_model\bert_model")
        self.tokenizer = tokenizer
        self.trained = False
        self.adapter = adapter
    ...
    def train(self, data, epochs, device='cpu', batch_size=32, lr=1e-5, load_model=None, lr_schedule=True):
    ...
    def predict(self, sentence, aspect, load_model=None, device='cpu'):
        ...
        input_tensor = torch.tensor([ids]).to('cpu')
        segment_tensor = torch.tensor(segment_tensor).to('cpu')
    ...
    def predict_batch(self, data, load_model=None, device='cpu'):
        ...
        tags_real = [str(t).strip('][').split(', ') for t in data['Tags']]
        tags_real = [[int(i) for i in t ] for t in tags_real]
        
        polarity_real = [str(t).strip('][').split(', ') for t in data['Polarities']]
        ...
        for i in tqdm(range(len(data))):
            ...
            for j in range(len(sentenceList)):
                if tags_real[i][j] != 0:
                    ...
                    w, p, _ = self.predict(sentence, aspect, load_model=load_model, device='cpu')
                    prediction.append(p)
                else:
    ...
    def accuracy(self, data, load_model=None, device='cpu'):
        a, p = self.test(data, load_model=load_model, device='cpu')
        return a